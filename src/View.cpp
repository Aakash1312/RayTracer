...
Ray
View::createViewingRay2(Sample const & s, int x, int y, double d) const
{
  Vec3 p = getSamplePosition(s);
...
void
View::createSample(Vec3 pos, Sample &s) const
{
  Vec3 d = (pos - eye_).normalize();
  double t = (LL_.z() - eye_.z())/(d.z());
  Vec3 result = eye_ + t*d;
  double sampleX = (result.x() - LL_.x())/(LR_.x() - LL_.x());
  double sampleY = (result.y() - LL_.y())/(UL_.y() - LL_.y());
  s.setX(sampleX);
  s.setY(sampleY);
}
