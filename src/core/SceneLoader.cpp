...
void SceneLoader::setLensDefaults(SceneGroup * n)
{
  if (n->lens_->material_ == NULL)
  {
    n->lens_->material_ = new ParametricMaterial();
    setMaterialDefaults(n->lens_->material_);
  }

  if (n->lens_->radius_ == NULL)
  {
    n->lens_->radius_ = new ConstValue(1);
  }

  if (n->lens_->width_ == NULL)
  {
    n->lens_->width_ = new ConstValue(1);
  }
...
bool SceneLoader::doLens(istream & str, string & name)
{
  ...
  if (!getName(str, "lens", name))
    return false;

  SceneGroup * n = new SceneGroup();
  groups[name] = n;
  n->name_ = name;
  n->lens_ = new ParametricLens();
    ...
    else if (state == OPEN)
    {
      ...
      if (readCommand(str, cmd))
      {
        ...
        else if (cmd == "material")
        {
          ...
          {
            n->lens_->material_ = materials[matName];
          }
        }
        else
        {
  ...
  while (findOpenParen(file))
  {
    ...
    if (readCommand(file, line))
    {
      ...
      else if (line == "Lens")
      {
        ...
        if (doLens(file, gname))
        {
          cout << "read lens " << gname << endl;
        }
        else
        {
          *err << "mangled lens command at ";
          errLine(file.tellg());
        }
      }
